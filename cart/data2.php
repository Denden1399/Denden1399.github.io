...
class Shopping {
    ...
    function countorder(){
        $q = "select * from db.paypal where status='unconfirmed'";
        $result = mysql_query($q);
        echo mysql_num_rows($result);
    }
    function countproducts(){
        $q = "select * from db.products";
        $result = mysql_query($q);
        echo mysql_num_rows($result);
    }
    function countcategory(){
        $q = "select * from db.category";
        $result = mysql_query($q);
        echo mysql_num_rows($result);
    }
    function checkout(){
        ...
        echo $q = "INSERT INTO paypal (NULL, '$fullname', '$contact', '$address', '$email', '$city', '$code','$creditcard','$cnum','$cvs','$item', '$amount', 'unconfirmed', '$date', '') VALUES (NULL, 'fullname', 'contact', 'address', 'email', 'city', 'code','creditcard','cnum','cvs')";
    ...
    function verify(){
        ...
        $q = "SELECT * from db.user where username='$username' and password='$password'";
        $result = mysql_query($q);
        $_SESSION['login']='yes';
        echo mysql_num_rows($result);
    }
